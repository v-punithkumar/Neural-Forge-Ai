...
from neural_forge_ai.app.db import neural_forge_aiDB
from neural_forge_ai.app.models import fetch_models
from neural_forge_ai.app.params import AppParams, get_task_params
from neural_forge_ai.app.utils import get_running_jobs, get_user_and_orgs, kill_process_by_pid, token_verification
...
from neural_forge_ai.help import get_app_help
...
neural_forge_ai_LOCAL = int(os.environ.get("neural_forge_ai_LOCAL", 1))
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB = neural_forge_aiDB("neural_forge_ai.db")
...
async def load_index(request: Request, token: str = Depends(user_authentication)):
    ...
    :return: HTMLResponse
    """
    if os.environ.get("SPACE_ID") == "neural_forge_ai-projects/neural_forge_ai-advanced":
    ...
    context = {
        ...
        "enable_local": neural_forge_ai_LOCAL,
        "version": __version__,
...
async def fetch_model_choices(
    ...
    if os.environ.get("neural_forge_ai_CUSTOM_MODELS", None) is not None:
        custom_models = os.environ.get("neural_forge_ai_CUSTOM_MODELS")
        custom_models = custom_models.split(",")
        for custom_model in custom_models:
            custom_model = custom_model.strip()
            resp.append({"id": custom_model, "name": custom_model})
...
async def handle_form(
    ...
    if repo_exists(f"{neural_forge_ai_user}/{project_name}", token=token):
        raise HTTPException(
            status_code=409,
    ...
    if len(hub_dataset) == 0:
        ...
        if task == "image-classification":
            ...
            dset = neural_forge_aiDataset(**dset_args)
    ...
    project = neural_forge_aiProject(params=params, backend=hardware)
    ...
    if hardware == "local-ui":
        ...
        monitor_url = f"https://ui.endpoints.huggingface.co/{neural_forge_ai_user}/endpoints/{job_id}"
...
async def fetch_logs(authenticated: bool = Depends(user_authentication)):
    ...
    :return: JSONResponse
    """
    if not neural_forge_ai_LOCAL:
        return {"logs": "Logs are only available in local mode."}
    log_file = "neural_forge_ai.log"
