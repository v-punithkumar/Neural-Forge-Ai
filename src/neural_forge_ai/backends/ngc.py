...
class NGCRunner(BaseBackend):
    ...
    def create(self):
        ...
        ngc_cmd = "set -x; conda run --no-capture-output -p /app/env neural_forge_ai api --port 7860 --host 0.0.0.0"
        ngc_payload = {
            "name": job_name,
            "aceName": NGC_ACE,
            "aceInstance": self.available_hardware[self.backend],
            "dockerImageName": f"{NGC_ORG}/neural_forge_ai-advanced:latest",
            "command": ngc_cmd,
