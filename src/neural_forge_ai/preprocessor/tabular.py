...
RESERVED_COLUMNS = ["neural_forge_ai_id", "neural_forge_ai_label"]
...
class TabularBinaryClassificationPreprocessor:
    ...
    Methods:
        ...
        prepare_columns(train_df, valid_df): Prepares the columns by adding 'neural_forge_ai_id' and 'neural_forge_ai_label', and drops the original ID and label columns.
    ...
    def prepare_columns(self, train_df, valid_df):
        train_df.loc[:, "neural_forge_ai_id"] = train_df[self.id_column] if self.id_column else list(range(len(train_df)))
        train_df.loc[:, "neural_forge_ai_label"] = train_df[self.label_column]
        valid_df.loc[:, "neural_forge_ai_id"] = valid_df[self.id_column] if self.id_column else list(range(len(valid_df)))
...
class TabularMultiLabelClassificationPreprocessor:
    ...
    Methods:
        ...
        prepare_columns(train_df, valid_df): Prepares the columns by adding neural_forge_ai_id and neural_forge_ai_label columns, and drops the original id_column and label_column.
    ...
    def prepare_columns(self, train_df, valid_df):
        ...
        for label in range(len(self.label_column)):
            train_df.loc[:, f"neural_forge_ai_label_{label}"] = train_df[self.label_column[label]]

        valid_df.loc[:, "neural_forge_ai_id"] = valid_df[self.id_column] if self.id_column else list(range(len(valid_df)))

        for label in range(len(self.label_column)):
            valid_df.loc[:, f"neural_forge_ai_label_{label}"] = valid_df[self.label_column[label]]
