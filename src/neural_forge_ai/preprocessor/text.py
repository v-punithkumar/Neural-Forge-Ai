...
RESERVED_COLUMNS = ["neural_forge_ai_text", "neural_forge_ai_label", "neural_forge_ai_question", "neural_forge_ai_answer"]
LLM_RESERVED_COLUMNS = [
    "neural_forge_ai_prompt",
    "neural_forge_ai_context",
    "neural_forge_ai_rejected_text",
    "neural_forge_ai_prompt_start",
]
...
class TextBinaryClassificationPreprocessor:
    ...
    def prepare_columns(self, train_df, valid_df):
        train_df.loc[:, "neural_forge_ai_text"] = train_df[self.text_column]
        train_df.loc[:, "neural_forge_ai_label"] = train_df[self.label_column]
        valid_df.loc[:, "neural_forge_ai_text"] = valid_df[self.text_column]
        valid_df.loc[:, "neural_forge_ai_label"] = valid_df[self.label_column]
    ...
    def prepare(self):
        ...
        train_df.loc[:, "neural_forge_ai_label"] = train_df["neural_forge_ai_label"].astype(str)
        valid_df.loc[:, "neural_forge_ai_label"] = valid_df["neural_forge_ai_label"].astype(str)

        label_names = sorted(set(train_df["neural_forge_ai_label"].unique().tolist()))

        train_df = Dataset.from_pandas(train_df)
        valid_df = Dataset.from_pandas(valid_df)

        if self.convert_to_class_label:
            train_df = train_df.cast_column("neural_forge_ai_label", ClassLabel(names=label_names))
            valid_df = valid_df.cast_column("neural_forge_ai_label", ClassLabel(names=label_names))
        ...
        if self.local:
            ...
            dataset.save_to_disk(f"{self.project_name}/neural_forge_ai-data")
        else:
            train_df.push_to_hub(
                f"{self.username}/neural_forge_ai-data-{self.project_name}",
                split="train",
                private=True,
                token=self.token,
            )
        ...
        if self.local:
            return f"{self.project_name}/neural_forge_ai-data"
        return f"{self.username}/neural_forge_ai-data-{self.project_name}"
...
class TextTokenClassificationPreprocessor(TextBinaryClassificationPreprocessor):
    ...
    def prepare(self):
        ...
        train_df, valid_df = self.prepare_columns(train_df, valid_df)
        try:
            train_df.loc[:, "neural_forge_ai_text"] = train_df["neural_forge_ai_text"].apply(lambda x: ast.literal_eval(x))
            valid_df.loc[:, "neural_forge_ai_text"] = valid_df["neural_forge_ai_text"].apply(lambda x: ast.literal_eval(x))
        except ValueError:
            logger.warning("Unable to do ast.literal_eval on train_df['neural_forge_ai_text']")
            logger.warning("assuming neural_forge_ai_text is already a list")
        try:
            train_df.loc[:, "neural_forge_ai_label"] = train_df["neural_forge_ai_label"].apply(lambda x: ast.literal_eval(x))
            valid_df.loc[:, "neural_forge_ai_label"] = valid_df["neural_forge_ai_label"].apply(lambda x: ast.literal_eval(x))
        except ValueError:
            logger.warning("Unable to do ast.literal_eval on train_df['neural_forge_ai_label']")
            logger.warning("assuming neural_forge_ai_label is already a list")

        label_names_train = sorted(set(train_df["neural_forge_ai_label"].explode().unique().tolist()))
        label_names_valid = sorted(set(valid_df["neural_forge_ai_label"].explode().unique().tolist()))
        ...
        if self.convert_to_class_label:
            train_df = train_df.cast_column("neural_forge_ai_label", Sequence(ClassLabel(names=label_names)))
            valid_df = valid_df.cast_column("neural_forge_ai_label", Sequence(ClassLabel(names=label_names)))
...
class LLMPreprocessor:
    ...
    def prepare_columns(self, train_df, valid_df):
        ...
        if self.prompt_column is not None:
            ...
            train_df.loc[:, "neural_forge_ai_prompt"] = train_df[self.prompt_column]
            valid_df.loc[:, "neural_forge_ai_prompt"] = valid_df[self.prompt_column]
        if self.rejected_text_column is not None:
            drop_cols.append(self.rejected_text_column)
            train_df.loc[:, "neural_forge_ai_rejected_text"] = train_df[self.rejected_text_column]
            valid_df.loc[:, "neural_forge_ai_rejected_text"] = valid_df[self.rejected_text_column]
...
class SentenceTransformersPreprocessor:
    ...
    def prepare_columns(self, train_df, valid_df):
        train_df.loc[:, "neural_forge_ai_sentence1"] = train_df[self.sentence1_column]
        train_df.loc[:, "neural_forge_ai_sentence2"] = train_df[self.sentence2_column]
        valid_df.loc[:, "neural_forge_ai_sentence1"] = valid_df[self.sentence1_column]
        valid_df.loc[:, "neural_forge_ai_sentence2"] = valid_df[self.sentence2_column]
        keep_cols = ["neural_forge_ai_sentence1", "neural_forge_ai_sentence2"]

        if self.sentence3_column is not None:
            train_df.loc[:, "neural_forge_ai_sentence3"] = train_df[self.sentence3_column]
            valid_df.loc[:, "neural_forge_ai_sentence3"] = valid_df[self.sentence3_column]
            keep_cols.append("neural_forge_ai_sentence3")

        if self.target_column is not None:
            train_df.loc[:, "neural_forge_ai_target"] = train_df[self.target_column]
            valid_df.loc[:, "neural_forge_ai_target"] = valid_df[self.target_column]
            keep_cols.append("neural_forge_ai_target")
    ...
    def prepare(self):
        ...
        if self.convert_to_class_label:
            label_names = sorted(set(train_df["neural_forge_ai_target"].unique().tolist()))

        train_df = Dataset.from_pandas(train_df)
        valid_df = Dataset.from_pandas(valid_df)

        if self.convert_to_class_label:
            train_df = train_df.cast_column("neural_forge_ai_target", ClassLabel(names=label_names))
            valid_df = valid_df.cast_column("neural_forge_ai_target", ClassLabel(names=label_names))
...
class TextExtractiveQuestionAnsweringPreprocessor:
    ...
    def prepare_columns(self, train_df, valid_df):
        train_df.loc[:, "neural_forge_ai_text"] = train_df[self.text_column]
        train_df.loc[:, "neural_forge_ai_question"] = train_df[self.question_column]
        train_df.loc[:, "neural_forge_ai_answer"] = train_df[self.answer_column]
        valid_df.loc[:, "neural_forge_ai_text"] = valid_df[self.text_column]
        valid_df.loc[:, "neural_forge_ai_question"] = valid_df[self.question_column]
        valid_df.loc[:, "neural_forge_ai_answer"] = valid_df[self.answer_column]

        # drop all other columns
        train_df = train_df.drop(
            columns=[
                x for x in train_df.columns if x not in ["neural_forge_ai_text", "neural_forge_ai_question", "neural_forge_ai_answer"]
            ]
        )
        valid_df = valid_df.drop(
            columns=[
                x for x in valid_df.columns if x not in ["neural_forge_ai_text", "neural_forge_ai_question", "neural_forge_ai_answer"]
            ]
        )
        return train_df, valid_df
