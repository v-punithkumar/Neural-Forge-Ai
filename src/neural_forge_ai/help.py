neural_forge_ai_user_info = """
<p>Please choose the user or organization who is creating the neural_forge_ai Project.</p>
<p>In case of non-free tier, this user or organization will be billed.</p>
"""

project_name_info = """A unique name for the neural_forge_ai Project.
This name will be used to identify the project in the neural_forge_ai dashboard."""

column_mapping_info = """
<p>Column Mapping is used to map the columns in the dataset to the columns in the neural_forge_ai Project.</p>
<p>For example, if your dataset has a column named "input" and you want to use it as the input for the model,
you can map it to the "text" column in the neural_forge_ai Project.</p>
<p>Similarly, if your dataset has a column named "label" and you want to use it as the label for the model,
you can map it to the "target" column in the neural_forge_ai Project.</p>
<p>Column mapping keys are neural_forge_ai Project column names and values are your dataset column names.</p>
...
Dont see your favorite model? You can also use a custom model by providing the model name in an environment variable: neural_forge_ai_CUSTOM_MODELS.
For example, go to settings and add a new environment variable with the key neural_forge_ai_CUSTOM_MODELS and value as the model name (e.g. google/gemma-7b)
"""
...
def get_app_help(element_id):
    if element_id == "neural_forge_ai_user_info":
        return neural_forge_ai_user_info
