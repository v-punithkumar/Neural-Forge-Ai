...
from neural_forge_ai import is_unsloth_available, logger
from neural_forge_ai.trainers.clm.callbacks import LoadBestPeftModelCallback, SavePeftModelCallback
...
# Model Trained Using neural_forge_ai

This model was trained using neural_forge_ai. For more information, please visit [neural_forge_ai](https://hf.co/docs/neural_forge_ai).
...
def create_model_card(config):
    ...
    if config.data_path == f"{config.project_name}/neural_forge_ai-data" or os.path.isdir(config.data_path):
        dataset_tag = ""
    else:
        dataset_tag = f"\ndatasets:\n- {config.data_path}"
